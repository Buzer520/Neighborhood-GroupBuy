...
Page({
  ...
  loadGroupBuyDetail: function() {
    ...
    db.collection('group_buy').doc(this.data.groupId).get()
      .then(res => {
        const groupBuy = res.data;
        const currentPrice = util.getCurrentPrice(groupBuy.tieredPrices, groupBuy.currentCount);
        const progress = util.getProgress(groupBuy.currentCount, groupBuy.targetCount);
        const isCreator = groupBuy.creatorId === app.globalData.openId;

        this.setData({
          groupBuy,
          currentPrice: currentPrice || groupBuy.targetPrice,
          progress,
          isCreator
        });
  ...
  loadCreatorInfo: function(creatorId) {
    db.collection('user').doc(creatorId).get()
      .then(res => {
  ...
  joinGroupBuy: function() {
    const { groupBuy, currentPrice, quantity, currentTab, hasJoined } = this.data;
    ...
    wx.cloud.callFunction({
      ...
      success: res => {
        if (res.result && res.result.success) {
          ...
          wx.showToast({ title: res.result?.message || '参与失败', icon: 'none' });
        }
      },
      fail: err => {
        console.error(err);
  ...
  goToShare: function() {
    ...
    wx.cloud.callFunction({
      ...
      success: res => {
        if (res.result) {
          wx.navigateTo({
            url: `/pages/share/share?scene=${encodeURIComponent(JSON.stringify(res.result))}`
          });
        }
      },
      fail: err => {
